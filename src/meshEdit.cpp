...
namespace CGL {
  ...
  void MeshEdit::drawHUD()
  {
    ...
    if(!selectedFeature.isValid())
    {
      ...
      if (v != NULL) {
        ostringstream m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12;
        m1 << "VERTEX DATA";
        m2 << "address      = " << v;
        ...
        y += inc;
        y += inc;
        drawString(x0, y, m10.str(), size, text_color);
        y += inc;
        drawString(x0, y, m11.str(), size, text_color);
        y += inc;
        drawString(x0, y, m12.str(), size, text_color);
        y += inc;
      }
